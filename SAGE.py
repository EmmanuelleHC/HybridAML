...
class TemporalGraphModelSAGEBatch(nn.Module):
    def __init__(self, embedding_dim, lstm_hidden_dim, lstm_layers, sage_hidden_dim, edge_attr_dim, num_nodes_global):
        super().__init__()
        ...
        self.sage = SAGEConv(embedding_dim, sage_hidden_dim)
        self.edge_predictor = nn.Linear(sage_hidden_dim * 2 + lstm_hidden_dim, 1)
    ...
    def forward(self, data):
        ...
        sage_out = self.sage(x, data.edge_index) 
        lstm_input = data.x.view(data.x.size(0), -1, 1)  
        lstm_out, _ = self.lstm(lstm_input)
        lstm_edge_out = lstm_out[:, -1, :]
        lstm_edge_out = lstm_edge_out[:data.edge_index.size(1), :]

        source_embeddings = sage_out[data.edge_index[0]]
        target_embeddings = sage_out[data.edge_index[1]]
