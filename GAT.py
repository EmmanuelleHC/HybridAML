...
class TemporalGraphModelGATBatch(nn.Module):
    def __init__(self, embedding_dim, lstm_hidden_dim, lstm_layers, gat_hidden_dim, edge_attr_dim, num_nodes_global, heads=1):
        super().__init__()
        ...
        self.gat = GATConv(embedding_dim, gat_hidden_dim, heads=heads, concat=True) 
        self.edge_predictor = nn.Linear((gat_hidden_dim * heads) * 2 + lstm_hidden_dim, 1)
    ...
    def forward(self, data):
        ...
        gat_out = self.gat(x, data.edge_index) 
        lstm_input = data.x.view(data.x.size(0), -1, 1)  
        lstm_out, _ = self.lstm(lstm_input)
        lstm_edge_out = lstm_out[:, -1, :] 
        lstm_edge_out = lstm_edge_out[:data.edge_index.size(1), :]  
        source_embeddings = gat_out[data.edge_index[0]]
        target_embeddings = gat_out[data.edge_index[1]]
