...
output_folder="Hybrid_GAT_LSTM"

writer = SummaryWriter(log_dir='Hybrid_GAT_LSTM/tensorboard_logs/')

logging.basicConfig(
    filename='Log/gat_consumer_hybrid_based.log',
    level=logging.INFO,
...
class TemporalGraphModelGATBatch(nn.Module):
    def __init__(self, embedding_dim, lstm_hidden_dim, lstm_layers, gnn_hidden_dim, edge_attr_dim, num_nodes_global, heads=2):
        ...
        self.gnn = GATConv(embedding_dim, gnn_hidden_dim, heads=heads, concat=True)  # Concat outputs of heads
...
def consume_and_evaluate(global_mapping, model_path, window_size=5000, output_folder="Hybrid_GAT_LSTM", timeout=3800, batch_size=500):
    ...
    model.eval()
    logging.info("GAT model loaded successfully.")
